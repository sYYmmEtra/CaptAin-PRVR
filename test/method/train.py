...
def train_epoch(model, train_loader, optimizer, opt, epoch_i, training=True):
    ...
    loss_meters = OrderedDict(clip_nce_loss=AverageMeter(), clip_trip_loss=AverageMeter(),
                            ...
                            local_hinge_loss=AverageMeter(),video_grounding_loss=AverageMeter(),
                            global_hinge_loss=AverageMeter(),global_nce_loss=AverageMeter(),
                            global_soft_pos_loss=AverageMeter(),global_soft_neg_loss=AverageMeter())
...
def start_training(opt):
    ...
    # caption
    # caption_files = {'train':f"",
    #                  'val':f"",}
    # Load visual features
    video2frames = None

    # train_dataset = Dataset4MS_SL(opt.caption_train_txt, None, text_feat_path, opt, video2frames=video2frames)
    ...
    model_config = EDict(
        ...
        # video_grounding_weight=opt.video_grounding_weight,
        # global_nce_weight=opt.global_nce_weight,
        # global_hinge_weight=opt.global_hinge_weight,
        # global_margin=opt.global_margin,
        # soft_pos_margin=opt.soft_pos_margin,
        # global_soft_pos_weight=opt.global_soft_pos_weight,
        # soft_neg_margin=opt.soft_neg_margin,
        # global_soft_neg_weight=opt.global_soft_neg_weight,
        query_or_caption=opt.query_or_caption,
        plot_losses=opt.plot_losses,
        # pretrain_grounding_epochs=opt.pretrain_grounding_epochs,
        # l1_or_mse=opt.l1_or_mse,
        # grounding_loss_only_in_pretrain=opt.grounding_loss_only_in_pretrain
    )
