...
class clip_nce(nn.Module):
    ...
    def forward(self,labels, label_dict, q2ctx_scores=None, contexts=None, queries=None):
        ...
        t2v_nominator = q2ctx_scores[diagnoal, labels] #取出了 q[i] 对 v[labels[i]] 的得分

        t2v_nominator = torch.logsumexp(t2v_nominator.unsqueeze(1), dim=1) #避免数值不稳定问题（相当于 softmax 分子）
        t2v_denominator = torch.logsumexp(q2ctx_scores, dim=1) # t2v 的分母（所有视频的得分） logsumexp 相当于 log(∑e^score)
